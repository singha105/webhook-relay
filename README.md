...
overloaded, slow, or briefly unreachable.

Doing it properly is not hard *code*, it is a pile of edge cases that have to be
handled somewhere: retrying without hammering a receiver into staying down,
giving up visibly rather than silently, recovering your own crashes without
spamming a receiver that already got the message, deciding what a *timeout* even
means, and proving months later what you actually sent.

Written inline, that logic gets duplicated in every service that sends
notifications, implemented differently each time, and tested by nobody. **This
extracts it into one component with one job.**

> 📐 **Full design documents:** [Problem statement](system-design/01-problem-statement.md) ·
> [High-level design](system-design/02-high-level-design.md) ·
> [Low-level design](system-design/03-low-level-design.md)

**Not** a message broker (no fan-out — use Kafka or NATS), not a task queue for
internal jobs, and not a human notification service. It delivers HTTP callbacks
to third-party endpoints, reliably and accountably, and nothing else.
...
**Ingest never touches the queue.** It writes to Postgres in one transaction and
returns 202. A separate relay polls for due events and is the only producer the
queue has — the transactional outbox pattern
([ADR 0002](docs/adr/0002-transactional-outbox.md)), and what makes the 202 mean
"durably stored" rather than "probably stored and probably queued".

The dashed arrow is what makes the system survivable. When a worker dies holding
entries, `XAUTOCLAIM` hands them to a live worker after the stale timeout. It is
also the arrow that causes duplicate deliveries, which is what the
[postmortem](docs/postmortem-duplicate-delivery.md) is about.

> 📐 **[High-level design →](system-design/02-high-level-design.md)** — system
> context and trust boundary, what each component is *not* allowed to touch, the
> request lifecycle, deployment topologies, what each choice costs, and where it
> breaks first.
...
| | |
|---|---|
| [**System design**](system-design/) | Problem statement, high-level and low-level design |
...
Schema, indexes, API contracts, the exact delivery headers, the signing
algorithm, retry classification, Valkey key layout, the concurrency model, every
configuration knob with what raising it costs, and a failure-mode table.

> 📐 **[Low-level design →](system-design/03-low-level-design.md)**

The short version of the part people ask about most: three tables, four indexes
each argued for individually, five event states with exactly two terminal ones,
and a delivery signed with `HMAC-SHA256` over `{timestamp}.{raw body}` carrying a
stable `X-Webhook-Id` across every retry.
